...
class LaporanTransaksiSimpananSeeder extends Seeder {
    public function run(): void {
        $a = DB::table("anggota")->pluck("id")->toArray();
        $u = DB::table("users")->pluck("id")->toArray();
        $k = DB::table("kantor")->pluck("id")->toArray();
        if (empty($a)) return;
        $j = ["setoran","tarikan","pemindahbukuan","penutupan"];
        $s = ["draft","posted","batal"];
        for ($i = 1; $i <= 100; $i++) {
            DB::table("laporan_transaksi_simpanan")->insert([
                "no_laporan" => "LS-".date("Ymd", strtotime("-{$i} days")).str_pad($i,4,"0",STR_PAD_LEFT),
                "tgl_laporan" => now()->subDays(rand(1,365))->format("Y-m-d"),
                "anggota_id" => $a[array_rand($a)],
                "jenis_transaksi" => $j[array_rand($j)],
                "nominal" => rand(5,200)*100000,
                "keterangan" => "Laporan transaksi simpanan #".$i,
