...
return new class extends Migration
{
    ...
    {
        Schema::create('pemindahbukuan_simpanan', function (Blueprint $table) {
            $table->id();
            $table->string('no_transaksi', 50)->unique();
            $table->date('tgl_transaksi');
            $table->foreignId('anggota_id')->constrained('anggota')->cascadeOnDelete();
            $table->foreignId('simpanan_dari_id')->constrained('simpanan')->cascadeOnDelete();
            $table->foreignId('simpanan_ke_id')->constrained('simpanan')->cascadeOnDelete();
    ...
    {
        Schema::dropIfExists('pemindahbukuan_simpanan');
    }
};
