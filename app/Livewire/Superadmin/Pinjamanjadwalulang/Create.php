...
namespace App\Livewire\Superadmin\Pinjamanjadwalulang;
...
class Create extends Component
{
    ...
    public $listSaksiTemp = [];

    // 2. Bedakan nama variabel untuk Modal Saksi agar tidak bentrok
    public $saksi_nama, $saksi_tempat_lahir, $saksi_tanggal_lahir, $saksi_nik, $saksi_pekerjaan, $saksi_alamat;

    public $editSaksiIndex = null;

    public function resetSaksiForm()
    {
        $this->reset(['saksi_nama', 'saksi_tempat_lahir', 'saksi_tanggal_lahir', 'saksi_nik', 'saksi_pekerjaan', 'saksi_alamat', 'editSaksiIndex']);
    }

    public function createSaksi()
    {
        $this->resetSaksiForm(); // Kosongkan form
        $this->dispatch('show-modal'); // Perintah buka modal
    }

    // Fungsi untuk Load Data ke Modal Edit
    public function editSaksi($index)
    {
        $this->editSaksiIndex = $index;
        $saksi = $this->listSaksiTemp[$index];

        $this->saksi_nama          = $saksi['nama'];
        $this->saksi_tempat_lahir  = $saksi['tempat_lahir'];
        $this->saksi_tanggal_lahir = $saksi['tanggal_lahir'];
        $this->saksi_nik           = $saksi['nik'];
        $this->saksi_pekerjaan     = $saksi['pekerjaan'];
        $this->saksi_alamat        = $saksi['alamat'];

        $this->dispatch('show-modal'); // Perintah buka modal setelah data terisi
    }

    // Fungsi menambah data ke Array
    public function saveSaksi()
    {
        $this->validate([
            'saksi_nama' => 'required',
            'saksi_nik'  => 'required',
        ...
        $dataSaksi = [
            'nama'          => $this->saksi_nama,
            'tempat_lahir'  => $this->saksi_tempat_lahir,
            'tanggal_lahir' => $this->saksi_tanggal_lahir,
            'nik'           => $this->saksi_nik,
            'pekerjaan'     => $this->saksi_pekerjaan,
            'alamat'        => $this->saksi_alamat,
        ];

        if ($this->editSaksiIndex !== null) {
            // Jika sedang mode edit, timpa (update) data di array pada index tersebut
            $this->listSaksiTemp[$this->editSaksiIndex] = $dataSaksi;
        } else {
            // Jika tidak, tambahkan sebagai data baru
            $this->listSaksiTemp[] = $dataSaksi;
        }
    ...
    // Fungsi menghapus dari Array
    public function removeSaksi($index)
    {
        unset($this->listSaksiTemp[$index]);
        // Re-index array agar urutannya (0,1,2..) tidak bolong
        $this->listSaksiTemp = array_values($this->listSaksiTemp);
    }

    // Fungsi merubah urutan ke atas
    public function moveUpSaksi($index)
    {
        if ($index > 0) {
            $temp = $this->listSaksiTemp[$index];
            $this->listSaksiTemp[$index] = $this->listSaksiTemp[$index - 1];
            $this->listSaksiTemp[$index - 1] = $temp;
        }
    }

    // Fungsi merubah urutan ke bawah
    public function moveDownSaksi($index)
    {
        if ($index < count($this->listSaksiTemp) - 1) {
            $temp = $this->listSaksiTemp[$index];
            $this->listSaksiTemp[$index] = $this->listSaksiTemp[$index + 1];
            $this->listSaksiTemp[$index + 1] = $temp;
        }
    ...
    public function render()
    {
        ...
        return view('livewire.superadmin.pinjamanjadwalulang.create', [
            'title' => 'Tambah Jadwal Ulang',
