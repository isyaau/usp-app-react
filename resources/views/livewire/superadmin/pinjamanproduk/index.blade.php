<div class="app-main">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start r gap-2">
                                <a wire:navigate href="/superadmin/pinjaman/produk/create" class="col-xl-2 col-md-2 btn btn-sm btn-primary mb-2"><i class="fa-solid fa-plus"></i> Tambah {{ $title }}</a>
                                ...
                                <table class="table table-hover table-bordered">
                                    <thead class="table-danger text-center">
                                        <tr>
                                            ...
                                            <th style="width: 300px">Nama Pinjaman</th>
                                            <th style="width: 100px;"><i class="fas fa-cog"></i></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($pinjaman_produk as $item)
                                        <tr class="align-middle">
                                            <td class="text-center">{{ $pinjaman_produk->firstItem() + $loop->index }}</td>
                                            <td>{{ $item->kode }}</td>
                                            <td>{{ $item->nama }}</td>
                                            <td class="text-center">
                                                <a wire:navigate href="/superadmin/pinjaman/produk/{{ $item->id }}/edit" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                                <a wire:navigate href="/superadmin/pinjaman/produk/{{ $item->id }}" class="btn btn-sm btn-primary"><i class="fas fa-eye"></i></a>
                                                <button type="button"
                                                    class="btn btn-sm btn-danger btn-delete-pinjaman_produk"
                                                    data-id="{{ $item->id }}">
    ...
    <script>
        ...
        function initDeleteButtons() {
            document.querySelectorAll('.btn-delete-pinjaman-produk').forEach(btn => {
