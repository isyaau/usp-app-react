<div class="app-main">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start r gap-2">
                                <a wire:navigate href="/superadmin/kantor/create" class="col-xl-2 col-md-2 btn btn-sm btn-primary mb-2"><i class="fa-solid fa-plus"></i> Tambah Kantor</a>
                                ...
                                <table class="table table-hover table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="align-middle">
                                            <td class="text-center">{{ $kantor->firstItem() + $loop->index }}</td>
                                            <td>{{ $item->kode }}</td>
                                            <td>{{ $item->nama_kantor }}</td>
                                            <td>{{ $item->alamat_kantor }}</td>

                                            <td class="text-center">
                                                <a wire:navigate href="/superadmin/kantor/{{ $item->id }}/edit" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                                <a wire:navigate href="/superadmin/kantor/{{ $item->id }}" class="btn btn-sm btn-primary"><i class="fas fa-eye"></i></a>
                                                <button type="button"
                                                    class="btn btn-sm btn-danger btn-delete-kantor"
                                                    data-id="{{ $item->id }}">
    ...
    @script
    <script>
        // SWEETALERT LISTENER (TIDAK BOLEH DIDALAM navigated)
        document.addEventListener('deleteSwal', () => {
            Swal.fire({
                title: 'Berhasil!',
        ...
        document.addEventListener('importSwal', () => {
            Swal.fire({
                title: 'Berhasil!',
        ...
        document.addEventListener('exportSwal', () => {
            Swal.fire({
                title: 'Berhasil!',
        ...
        document.addEventListener('importErrorSwal', (event) => {
            Swal.fire({
                title: 'Gagal!',
                text: event.detail.message ?? 'Terjadi kesalahan saat import.',
                icon: 'error'
            });
        });
        ...
        function initDeleteButtons() {
            document.querySelectorAll('.btn-delete-kantor').forEach(btn => {
                btn.onclick = function() {
                    let kantorId = this.dataset.id;
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            Livewire.dispatch('deleteKantor', {
                                id: kantorId
                            });
                        }
                    });
        ...
        // WAJIB! setiap navigasi / update Livewire, tombol harus diregister ulang
        document.addEventListener('livewire:navigated', initDeleteButtons);
        document.addEventListener('livewire:update', initDeleteButtons);
    </script>
    @endscript
