<div class="app-main">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start r gap-2">
                                <a wire:navigate href="/superadmin/account-header/create" class="col-xl-2 col-md-2 btn btn-sm btn-primary mb-2"><i class="fa-solid fa-plus"></i> Tambah {{ $title }}</a>
                                ...
                                <table class="table table-hover table-bordered">
                                    <thead class="table-danger text-center">
                                        <tr>
                                            ...
                                            <th style="width: 100px">No Header</th>
                                            <th style="width: 300px">Nama Header</th>
                                    ...
                                    <tbody>
                                        @foreach ($accheader as $item)
                                        <tr class="align-middle">
                                            <td class="text-center">{{ $accheader->firstItem() + $loop->index }}</td>
                                            <td>{{ $item->no_header }}</td>
                                            <td>{{ $item->nama }}</td>
                                            <td>{{ $item->group ? $item->group->nama : '-' }}</td>
                                            <td>{{ $item->jenis }}</td>
                                            <td class="text-center">
                                                <a wire:navigate href="/superadmin/account-header/{{ $item->id }}/edit" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                                <a wire:navigate href="/superadmin/account-header/{{ $item->id }}" class="btn btn-sm btn-primary"><i class="fas fa-eye"></i></a>
                                                <button type="button"
                                                    class="btn btn-sm btn-danger btn-delete-accheader"
                                                    data-id="{{ $item->id }}">
    ...
    <script>
        ...
        function initDeleteButtons() {
            document.querySelectorAll('.btn-delete-accheader').forEach(btn => {
                btn.onclick = function() {
                    let kelompokId = this.dataset.id;
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            Livewire.dispatch('deleteAccheader', {
                                id: kelompokId
                            });
                        }
                    });
