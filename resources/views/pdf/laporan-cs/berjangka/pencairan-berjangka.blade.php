...
<body>
    ...
    <div class="content">
        <h2>Laporan Pencairan Simpanan Berjangka</h2>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ optional($item->tgl_transaksi)->format('d/m/Y') ?? '—' }}</td>
                    <td>{{ $item->deposito->no_deposito ?? '—' }}</td>
                    <td>{{ $item->anggota->no_anggota ?? '—' }}</td>
                    <td>{{ $item->anggota->nama ?? '—' }}</td>
                    <td>{{ $item->deposito->produk->nama ?? '—' }}</td>
                    <td style="text-align:right;">Rp {{ number_format($item->nominal_pokok ?? 0, 0, ',', '.') }}</td>
                    <td style="text-align:right;">Rp {{ number_format($item->nominal_bunga ?? 0, 0, ',', '.') }}</td>
                    <td style="text-align:right;">Rp {{ number_format($item->nominal_pajak ?? 0, 0, ',', '.') }}</td>
                    <td style="text-align:right;">Rp {{ number_format($item->nominal_penalti ?? 0, 0, ',', '.') }}</td>
                    <td style="text-align:right;">Rp {{ number_format($item->nominal_diterima ?? 0, 0, ',', '.') }}</td>
                </tr>
                @empty
