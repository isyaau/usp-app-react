...
import type { AccountMini, DepositoJenisDetail } from '@/types/models';
...
export default function ProdukEdit({ produkData, accountOptions }: Props) {
    const form = useForm<FormValues>({
        kode: produkData.kode,
        nama: produkData.nama,
        account_id: String(produkData.account_id ?? ''),
        jangka_waktu: produkData.jangka_waktu ?? '',
        bunga: produkData.bunga ?? '',
        account_bunga: String(produkData.account_bunga ?? ''),
        rumus_bunga: produkData.rumus_bunga ?? '',
        penalti: produkData.penalti ?? '',
        account_penalti: String(produkData.account_penalti ?? ''),
        pajak: produkData.pajak ?? '',
        account_pajak: String(produkData.account_pajak ?? ''),
        saldo_pajak: produkData.saldo_pajak ?? '',
        insentif: produkData.insentif ?? '',
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        form.put(route('superadmin.simpanan-berjangka.produk.update', produkData.id));
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <PageHeader
                title="Edit Produk Berjangka"
                description={produkData.nama}
                icon={Package}
