import { useState } from 'react';
import { Link, Head, router } from '@inertiajs/react';
import { Eye, FileText, Pencil, Plus, Printer, Search } from 'lucide-react';

import AuthenticatedLayout from '@/Layouts/AuthenticatedLayout';
import { PageHeader } from '@/Components/PageHeader';
import { Pagination } from '@/Components/Pagination';
import { ConfirmDelete } from '@/Components/ConfirmDelete';
import { Button } from '@/Components/ui/button';
import { Card } from '@/Components/ui/card';
import { Input } from '@/Components/ui/input';
import {
    Select,
    SelectContent,
    SelectItem,
    SelectTrigger,
    SelectValue,
} from '@/Components/ui/select';
import {
    Table,
    TableBody,
    TableCell,
    TableHead,
    TableHeader,
    TableRow,
} from '@/Components/ui/table';
import type { Paginated, ProposalRow } from '@/types/models';

interface Props {
    proposal: Paginated<ProposalRow>;
    filters: { search: string };
}

const rupiah = (v: string | number | null) =>
    `Rp ${Number(v ?? 0).toLocaleString('id-ID')}`;

export default function ProposalIndex({ proposal, filters }: Props) {
    const [search, setSearch] = useState(filters.search ?? '');
    const [perPage, setPerPage] = useState(String(proposal.per_page));

    const apply = (overrides: { search?: string; per_page?: string } = {}) => {
        router.get(
            route('superadmin.pinjaman.proposal'),
            {
                search: overrides.search ?? search,
                per_page: overrides.per_page ?? perPage,
            },
            { preserveState: true, preserveScroll: true },
        );
    };

    return (
        <AuthenticatedLayout>
            <Head title="Data Proposal Pinjaman" />

            <PageHeader
                title="Data Proposal Pinjaman"
                description="Kelola proposal pinjaman anggota."
                icon={FileText}
            >
                <Button asChild className="bg-brand-600 hover:bg-brand-500">
                    <Link href={route('superadmin.pinjaman.proposal.create')} preload="hover">
                        <Plus />
                        Tambah Proposal
                    </Link>
                </Button>
            </PageHeader>

            <Card className="gap-4 py-5">
                <div className="flex flex-wrap items-center gap-3 px-5">
                    <div className="relative min-w-56 flex-1">
                        <Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                        <Input
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            onKeyDown={(e) => e.key === 'Enter' && apply()}
                            placeholder="Cari no. bukti / debitur…"
                            className="pl-9"
                        />
                    </div>
                    <Select
                        value={perPage}
                        onValueChange={(v) => {
                            setPerPage(v);
                            apply({ per_page: v });
                        }}
                    >
                        <SelectTrigger className="w-28">
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                            {['10', '25', '50', '100'].map((n) => (
                                <SelectItem key={n} value={n}>
                                    {n} / hal.
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                </div>

                <div className="px-5 overflow-x-auto">
                    <Table>
                        <TableHeader>
                            <TableRow>
                                <TableHead className="w-12">#</TableHead>
                                <TableHead>Tanggal</TableHead>
                                <TableHead>No. Bukti</TableHead>
                                <TableHead>Produk</TableHead>
                                <TableHead>Debitur</TableHead>
                                <TableHead>Jangka Waktu</TableHead>
                                <TableHead>Bunga</TableHead>
                                <TableHead>Plafon</TableHead>
                                <TableHead>Total Terima</TableHead>
                                <TableHead className="text-right">Aksi</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {proposal.data.length === 0 && (
                                <TableRow>
                                    <TableCell
                                        colSpan={10}
                                        className="h-32 text-center text-muted-foreground"
                                    >
                                        Tidak ada data proposal.
                                    </TableCell>
                                </TableRow>
                            )}
                            {proposal.data.map((item, i) => (
                                <TableRow key={item.id}>
                                    <TableCell className="text-muted-foreground">
                                        {proposal.from !== null ? proposal.from + i : i + 1}
                                    </TableCell>
                                    <TableCell className="whitespace-nowrap text-muted-foreground">
                                        {item.tanggal}
                                    </TableCell>
                                    <TableCell>
                                        <span className="rounded-md bg-muted px-2 py-0.5 font-mono text-xs">
                                            {item.no_bukti}
                                        </span>
                                    </TableCell>
                                    <TableCell>{item.jenis_pinjaman?.nama ?? '—'}</TableCell>
                                    <TableCell>
                                        {item.anggota ? (
                                            <>
                                                {item.anggota.nama}
                                                <span className="block font-mono text-xs text-muted-foreground">
                                                    {item.anggota.no_anggota}
                                                </span>
                                            </>
                                        ) : (
                                            '—'
                                        )}
                                    </TableCell>
                                    <TableCell className="whitespace-nowrap text-muted-foreground">
                                        {item.jangka_waktu} {item.satuan}
                                    </TableCell>
                                    <TableCell>{item.bunga}%</TableCell>
                                    <TableCell className="font-mono">{rupiah(item.plafon)}</TableCell>
                                    <TableCell className="font-mono">{rupiah(item.total_terima)}</TableCell>
                                    <TableCell>
                                        <div className="flex items-center justify-end gap-1">
                                            <Button variant="ghost" size="icon" asChild title="Detail">
                                                <Link href={route('superadmin.pinjaman.proposal.show', item.id)}>
                                                    <Eye className="text-muted-foreground" />
                                                </Link>
                                            </Button>
                                            <Button variant="ghost" size="icon" asChild title="Cetak">
                                                <Link href={route('superadmin.pinjaman.proposal.cetak', item.id)}>
                                                    <Printer className="text-muted-foreground" />
                                                </Link>
                                            </Button>
                                            <Button variant="ghost" size="icon" asChild title="Edit">
                                                <Link href={route('superadmin.pinjaman.proposal.edit', item.id)}>
                                                    <Pencil className="text-muted-foreground" />
                                                </Link>
                                            </Button>
                                            <ConfirmDelete
                                                routeName="superadmin.pinjaman.proposal.destroy"
                                                id={item.id}
                                                label={item.no_bukti}
                                            />
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
                </div>

                <div className="border-t px-5 pt-4">
                    <Pagination
                        links={proposal.links}
                        currentPage={proposal.current_page}
                        lastPage={proposal.last_page}
                        from={proposal.from}
                        to={proposal.to}
                        total={proposal.total}
                    />
                </div>
            </Card>
        </AuthenticatedLayout>
    );
}
