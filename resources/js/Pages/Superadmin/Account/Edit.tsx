...
export default function AccountEdit({ accountData, headers }: Props) {
    const form = useForm<FormValues>({
        no_account: accountData.no_account,
        nama: accountData.nama,
        header_id: String(accountData.header_id),
        tipe: accountData.tipe,
    });

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        form.put(route('superadmin.account.update', accountData.id));
    };

    return (
        <AuthenticatedLayout>
            <Head title={`Edit ${accountData.nama}`} />

            <PageHeader
                title="Edit Account"
                description={`Perbarui data ${accountData.nama}.`}
                icon={Pencil}
