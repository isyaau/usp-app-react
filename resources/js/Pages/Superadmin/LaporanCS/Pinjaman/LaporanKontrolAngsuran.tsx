...
export default function LaporanKontrolAngsuran({ data, filters, kantors, variantTitle }: Props) {
    return (
        <AuthenticatedLayout>
            <Head title={variantTitle || 'Laporan Kontrol Angsuran'} />
            <PageHeader title={variantTitle || 'Laporan Kontrol Angsuran'} description="Laporan kontrol angsuran pinjaman." icon={Eye} />
            <Card className="gap-4 py-5">
                <div className="px-5">
                    <ReportFilterBar
                        routeName="superadmin.laporan-cs.pinjaman.kontrol-angsuran"
                        filters={filters}
                        showDateRange={true}
                        showKantor={true}
                        kantors={kantors}
                        printRoute="superadmin.laporan-cs.pinjaman.kontrol-angsuran.cetak"
                    />
                </div>
                <div className="px-5 overflow-x-auto">
                    <Table>
                        ...
                        <TableBody>
                            {data.data.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={13} className="h-32 text-center text-muted-foreground">Tidak ada data.</TableCell>
                                </TableRow>
                            ) : (
                                data.data.map((item, i) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>{item.angsuranke}/{item.jangka_waktu}</TableCell>
