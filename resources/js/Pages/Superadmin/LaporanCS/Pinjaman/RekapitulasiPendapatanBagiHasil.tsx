...
export default function RekapitulasiPendapatanBagiHasil({ data, filters, kantors, variantTitle }: Props) {
    return (
        <AuthenticatedLayout>
            <Head title={variantTitle || 'Rekapitulasi Pendapatan Bagi Hasil'} />
            <PageHeader title={variantTitle || 'Rekapitulasi Pendapatan Bagi Hasil'} description="Rekapitulasi pendapatan bagi hasil pinjaman." icon={PieChart} />
            <Card className="gap-4 py-5">
                <div className="px-5">
                    <ReportFilterBar
                        routeName="superadmin.laporan-cs.pinjaman.rekap-bagi-hasil"
                        filters={filters}
                        showDateRange={true}
                        showKantor={true}
                        kantors={kantors}
                        printRoute="superadmin.laporan-cs.pinjaman.rekap-bagi-hasil.cetak"
                    />
                </div>
                            ...
                            {data.data.length === 0 ? (
                                ...
                                data.data.map((item, i) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="text-right">{Number(item.total_angsuran).toLocaleString('id-ID')}</TableCell>
