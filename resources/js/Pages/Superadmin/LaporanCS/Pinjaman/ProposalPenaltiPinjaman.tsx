...
export default function ProposalPenaltiPinjaman({ data, filters, kantors, variantTitle }: Props) {
    return (
        <AuthenticatedLayout>
            <Head title={variantTitle || 'Proposal Penalti Pinjaman'} />
            <PageHeader title={variantTitle || 'Proposal Penalti Pinjaman'} description="Daftar proposal penalti pinjaman." icon={AlertTriangle} />
            <Card className="gap-4 py-5">
                <div className="px-5">
                    <ReportFilterBar
                        routeName="superadmin.laporan-cs.pinjaman.proposal-penalti"
                        filters={filters}
                        showDateRange
                        showKantor
                        kantors={kantors}
                        printRoute="superadmin.laporan-cs.pinjaman.proposal-penalti.cetak"
                    />
                </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="text-right">Nominal Penalti</TableHead>
                            ...
                            {data.data.length === 0 ? (
                                ...
                                data.data.map((item, i) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="text-right">{Number(item.nominal_penalti).toLocaleString('id-ID')}</TableCell>
