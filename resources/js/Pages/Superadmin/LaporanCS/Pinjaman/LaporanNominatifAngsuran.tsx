...
interface NominatifAngsuranRow {
    id: number;
    no_pinjaman: string;
    no_anggota: string;
    nama: string;
...
interface Props {
    data: Paginated<NominatifAngsuranRow>;
...
export default function LaporanNominatifAngsuran({ data, filters, kantors, variantTitle }: Props) {
    return (
        <AuthenticatedLayout>
            <Head title={variantTitle || 'Laporan Nominatif Angsuran'} />

            <PageHeader
                title={variantTitle || 'Laporan Nominatif Angsuran'}
                description="Laporan nominatif angsuran pinjaman anggota."
                icon={FileText}
            />
            ...
            <Card className="gap-4 py-5">
                <div className="px-5">
                    <ReportFilterBar
                        routeName="superadmin.laporan-cs.pinjaman.nominatif-angsuran"
                        filters={filters}
                        kantors={kantors}
                        showKantor
                        printRoute="superadmin.laporan-cs.pinjaman.nominatif-angsuran.cetak"
                    />
                </div>
                        ...
                        <TableBody>
                            {data.data.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={12} className="h-32 text-center text-muted-foreground">
                                        Tidak ada data nominatif angsuran.
                                    </TableCell>
                                </TableRow>
                            )}
                            {data.data.map((item, i) => (
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell className="text-muted-foreground">
                                        {Number(item.nominal_angsuran).toLocaleString('id-ID')}
                                    </TableCell>
                                    <TableCell className="text-muted-foreground">
                                        {item.angsuranke}
                                    </TableCell>
