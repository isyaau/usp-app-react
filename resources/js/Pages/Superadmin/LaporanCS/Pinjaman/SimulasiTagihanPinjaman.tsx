...
export default function SimulasiTagihanPinjaman({ data, filters, kantors, variantTitle }: Props) {
    return (
        <AuthenticatedLayout>
            <Head title={variantTitle || 'Simulasi Tagihan Pinjaman'} />
            <PageHeader title={variantTitle || 'Simulasi Tagihan Pinjaman'} description="Simulasi tagihan angsuran pinjaman." icon={Calculator} />
            <Card className="gap-4 py-5">
                <div className="px-5">
                    <ReportFilterBar
                        routeName="superadmin.laporan-cs.pinjaman.simulasi-tagihan"
                        filters={filters}
                        showKantor={true}
                        kantors={kantors}
                        printRoute="superadmin.laporan-cs.pinjaman.simulasi-tagihan.cetak"
                    />
                </div>
                            ...
                            {data.data.length === 0 ? (
                                ...
                                data.data.map((item, i) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="text-right">{Number(item.angsuran_bulan).toLocaleString('id-ID')}</TableCell>
