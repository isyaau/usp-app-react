...
export default function LaporanPendapatanBagiHasil({ data, filters, kantors, variantTitle }: Props) {
    return (
        <AuthenticatedLayout>
            <Head title={variantTitle || 'Laporan Pendapatan Bagi Hasil'} />
            <PageHeader title={variantTitle || 'Laporan Pendapatan Bagi Hasil'} description="Laporan pendapatan bagi hasil pinjaman." icon={DollarSign} />
            <Card className="gap-4 py-5">
                <div className="px-5">
                    <ReportFilterBar
                        routeName="superadmin.laporan-cs.pinjaman.bagi-hasil"
                        filters={filters}
                        showDateRange
                        showKantor
                        kantors={kantors}
                        printRoute="superadmin.laporan-cs.pinjaman.bagi-hasil.cetak"
                    />
                </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="text-right">Bunga (pendapatan)</TableHead>
                            ...
                            {data.data.length === 0 ? (
                                ...
                                data.data.map((item, i) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell className="text-right">{Number(item.bunga_pendapatan).toLocaleString('id-ID')}</TableCell>
