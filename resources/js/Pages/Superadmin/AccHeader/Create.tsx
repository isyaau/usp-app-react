...
/** Opsi jenis per grup — replikasi logika getRadioByGroup Livewire lama. */
...
export default function AccHeaderCreate({ groups }: Props) {
    ...
    const radioItems = useMemo(
        () => (selectedGroupName ? jenisOptions(selectedGroupName) : {}),
        [selectedGroupName],
    );

    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        form.post(route('superadmin.account-header.store'));
    };

    return (
        <AuthenticatedLayout>
            <Head title="Tambah Account Header" />

            <PageHeader
                title="Tambah Account Header"
                description="Buat header akun baru dalam struktur COA."
                icon={Bookmark}
            ...
            <form onSubmit={submit} className="max-w-2xl">
                <Card>
                    ...
                    <CardContent className="space-y-5">
                        ...
                        {/* Radio Jenis — tampil setelah grup dipilih */}
