...
import type { Paginated, PengembalianJaminanRow } from '@/types/models';

interface Props {
    transaksi: Paginated<PengembalianJaminanRow>;
    filters: { search: string; status: string; mulai: string; sampai: string };
}

const STATUS_BADGE: Record<PengembalianJaminanRow['status'], string> = {
...
export default function Index({ transaksi, filters }: Props) {
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Pengembalian Jaminan" />

            <PageHeader
                title="Pengembalian Jaminan"
                description="Catatan pengembalian jaminan atas pinjaman yang sudah selesai/lunas."
                icon={ShieldCheck}
            >
                <Button asChild className="bg-brand-600 hover:bg-brand-500">
                    <Link href={route(`${root}.create`)}>Tambah Pengembalian Jaminan</Link>
                </Button>
            </PageHeader>
            ...
            <Card className="mt-5">
                <CardHeader>
                    <CardTitle className="text-base">Daftar Pengembalian Jaminan</CardTitle>
                </CardHeader>
                <CardContent>
                    <Table>
                        <TableHeader>
                            ...
                            {transaksi.data.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={8} className="py-10 text-center text-muted-foreground">
                                        Tidak ada data pengembalian jaminan.
                                    </TableCell>
                                </TableRow>
                            )}
                            {transaksi.data.map((item, i) => (
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell className="font-mono">
                                        Rp {Number(item.sisa_pokok).toLocaleString('id-ID')}
                                    </TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell className="space-x-1 whitespace-nowrap text-right">
                                        ...
                                        <ConfirmDelete
                                            ...
                                            description={`Pengembalian jaminan "${item.no_transaksi}" akan dihapus permanen.`}
                                        />
                                    </TableCell>
                                </TableRow>
                            ))}
